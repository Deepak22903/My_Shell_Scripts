...
rsyncy -av -h --mkpath --no-relative --exclude='IntelliJ*' --exclude='Large_Files*' --exclude='Genshin*' --exclude='Fooocus*' --exclude='AI_Lab*' --exclude='node_modules/' --exclude='Adobe*' --exclude='.git/' --exclude='env/' --exclude='.*' --max-size=0.5G "$source_path/" "$destination_path"
