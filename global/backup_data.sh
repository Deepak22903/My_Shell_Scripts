...
rsyncy -av -h --mkpath --no-relative --exclude='AI_Lab*' --exclude='node_modules/' --exclude='Adobe*' --exclude='.git/' --exclude='env/' --exclude='.*' --max-size=1G "$source_path/" "$destination_path"
